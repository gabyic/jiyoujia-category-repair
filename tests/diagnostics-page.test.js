...
test("完整诊断返回修复状态和链接统计且不读取浏览器私有存储", () => {
  ...
  assert.equal(result.shopContext.hostType, "numeric");
  assert.deepEqual(result.shopContext.signals, ["numeric-shop-host"]);
...
test("诊断器与公共识别器使用相同店铺判定", () => {
  ...
  assert.equal(
    isSupportedCategoryLocation(
      { hostname: "custom.taobao.com", pathname: "/category.htm" },
      customShopDocument
    ),
    true
  );
...
test("未确认的自定义域名仍返回可排查的识别原因", () => {
  const body = { innerText: "", scrollHeight: 0 };
  const documentLike = {
    title: "淘宝网",
    readyState: "complete",
    body,
  ...
  const windowLike = {
    location: {
      hostname: "random.taobao.com",
      pathname: "/category.htm",
      href: "https://random.taobao.com/category.htm",
    },
    innerWidth: 1280,
    innerHeight: 720,
    devicePixelRatio: 1,
  ...
  assert.equal(result.shopContext.hostType, "custom");
  assert.equal(result.shopContext.reason, "shop-signals-not-found");
  assert.deepEqual(result.shopContext.signals, ["shop-title"]);
});
